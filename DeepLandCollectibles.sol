...
abstract contract BlackList {
    ...
    function totalBlackList() external view returns (uint256) {
    ...
    BlackList
{
    ...
    constructor(address owner, address signer)
        ERC721("DeepLand", "DeepLand")
    {
        _grantRole(DEFAULT_ADMIN_ROLE, owner);
        _grantRole(PAUSER_ROLE, owner);
        _grantRole(MINTER_ROLE, owner);
        _grantRole(SIGNER_ROLE, signer);
        _grantRole(BLOCKER_ROLE, owner);
    }
    ...
    function lazyMint(
        ...
        require(
            hasRole(SIGNER_ROLE, getSigner(to, id, tokenUri, data, signature)),
    ...
    function getSigner(
        ...
        bytes32 hash = prepareHash(to, ids, tokenUri, data)
...
contract FACTORY is AccessControlEnumerable, BlackList, Pausable {
    bytes32 public constant BLOCKER_ROLE = keccak256("BLOCKER_ROLE");
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    ...
    function deploy(address account, address signer) external whenNotPaused {
        require(
            account != address(0) &&
                signer != address(0) &&
                !Address.isContract(account) &&
                !Address.isContract(signer),
            "deploy: Invalid Address"
        );
        require(!isBlackListed(account), "deploy: account is BlackListed");
        require(!isBlackListed(signer), "deploy: signer is BlackListed");
        require(
            !isBlackListed(_msgSender()),
            "deploy: msgSender is BlackListed"
        );

        bytes memory bytecode = type(DeepLandNFT).creationCode;
        bytes memory constructorArgs = abi.encode(account, signer);
        ...
        address deployedContract = Create2.deploy(
            0,
            keccak256(abi.encodePacked(_factory[account].length, account)),
            deploymentData
        );

        if (_factory[account].length == 0) _users.push(account);
        _factory[account].push(deployedContract);

        emit Deploy(account, deployedContract, signer, block.timestamp);
    }
